...
var _default = function _default(options) {
  ...
  var Provider = function Provider(props) {
    ...
    var dispatchAction = useMemo(function () {
      return initDispatchAction(dispatch, actions, state, getters);
    }, [actions, getters, state]); // dispatchAction没法做到引用保持不变 所以挂到dispatch上
    // 这样用户使用useEffect把dispatch作为依赖 就不会造成无限更新
    // @ts-ignore

    dispatch.action = dispatchAction; // 给loadingMap加上一些api
